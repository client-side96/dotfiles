...
function status.apply_to_config(window, pane)
	...
	if key_table_name then
		return window:set_right_status(wezterm.format({
			{ Foreground = { Color = colors_config.crust } },
			{ Background = { Color = colors_config.red } },
			{ Text = (key_table_name or "") },
		}))
	end
	if workspace_name then
		local bg_color = string.find(workspace_name, "ssh") and colors_config.red or colors_config.blue
		return window:set_right_status(wezterm.format({
			{ Foreground = { Color = colors_config.crust } },
